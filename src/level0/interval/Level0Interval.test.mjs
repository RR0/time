...
describe("Level0Interval", () => {
  ...
  describe("rendering", () => {
    ...
    test("default", () => {
      const start = "1985"
      const end = new Level0Date({ year: 2001 })
      const interval = new Level0Interval(start, end)
      assert.equal(interval.toString(), interval.start.year.toString() + "/" + interval.end.year.toString())
    })
    ...
    test("custom", () => {
      ...
      const end = new Level0Date({ year: 2001 })
      const interval = new Level0Interval(start, end)
      const customRenderer = new class extends Level0IntervalRenderer {
        render (interval) {
          return "Between " + interval.start.year.toString() + " and " + interval.end.year.toString()
        }
      }()
      assert.equal(interval.toString(customRenderer), "Between " + start + " and " + end.year.value)
    })
  })
